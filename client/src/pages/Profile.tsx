        ...
        <div className="lg:col-span-3">
          ...
          {isOrdersLoading ? (
            ...
            <div className="space-y-4">
              {orders.map((order) => (
                <div key={order.id} className={`border border-border p-4 md:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-black transition-colors ${order.status === 'paid' ? 'bg-emerald-50/30' :
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 md:gap-4 flex-1 w-full">
                    <div className="flex items-center justify-between sm:justify-start gap-3 w-full sm:w-auto">
                      ...
                      <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-sm whitespace-nowrap ${order.status === 'delivered' ? 'bg-green-100 text-green-800' :
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 md:gap-8 w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-border">
                    <span className="font-bold text-sm md:text-base">₹{Number(order.total_amount).toFixed(2)}</span>
